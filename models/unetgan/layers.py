...
class ccbn(nn.Module):
    def __init__(self, output_size, input_size, which_linear, eps=1e-5, momentum=0.1,
        ...
        if self_modulation:
            self.gain = nn.Sequential(which_linear(input_size, input_size, bias=True), nn.ReLU(),
                                      which_linear(input_size, output_size, bias=False))
            self.bias = nn.Sequential(which_linear(input_size, input_size, bias=True), nn.ReLU(),
                                      which_linear(input_size, output_size, bias=False))
        else:
